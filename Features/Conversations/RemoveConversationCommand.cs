...
namespace NoDaysOffApp.Features.Conversations
{
    public class RemoveConversationCommand
    {
        ...
        public class Handler : IAsyncRequestHandler<Request, Response>
        {
            ...
            {
                var conversation = await _context.Conversations.SingleAsync(x=>x.Id == request.Id && x.Tenant.UniqueId == request.TenantUniqueId);
                conversation.IsDeleted = true;
                await _context.SaveChangesAsync();
                _bus.Publish(new RemovedConversationMessage(request.Id, request.CorrelationId, request.TenantUniqueId));
                return new Response();
            }
