...
@Component({
  ...
  template: `
    <div class="dialog" data-testid="athlete-dialog">
      <div class="dialog-header">
        <h2>{{ isEdit ? 'Edit Athlete' : 'Add Athlete' }}</h2>
      ...
      <div class="dialog-content">
        <mat-form-field appearance="outline" class="full-width">
          ...
          <input matInput [(ngModel)]="name" data-testid="athlete-name-input" required>
        </mat-form-field>
        <mat-form-field appearance="outline" class="full-width">
          <mat-label>Username</mat-label>
          <input matInput [(ngModel)]="username" data-testid="athlete-username-input" required>
        </mat-form-field>
        @if (error()) {
...
export class AthleteDialog {
  private readonly dialogRef = inject(MatDialogRef<AthleteDialog>);
  private readonly data = inject<Athlete | null>(MAT_DIALOG_DATA, { optional: true });
  private readonly athletesService = inject(AthletesService);

  isEdit = !!this.data?.athleteId;
  name = this.data?.name ?? '';
  ...
  save() {
    if (!this.name || !this.username) { this.error.set('All fields are required'); return; }
    this.loading.set(true);
    const obs = this.isEdit
      ? this.athletesService.updateAthlete(this.data!.athleteId, { athleteId: this.data!.athleteId, name: this.name, username: this.username })
      : this.athletesService.createAthlete({ name: this.name, username: this.username });
    obs.subscribe({
